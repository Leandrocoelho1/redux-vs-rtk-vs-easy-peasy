...
type SelectedTodoReducerActionTypes = SelectTodoActionType;
const selectedTodoReducer = (
  state: string | null = null,
  action: SelectedTodoReducerActionTypes
): string | null => {
  switch (action.type) {
    case SELECT_TODO: {
      const { payload } = action;
...
const counterReducer = (state = 0, action: TodosReducerActionTypes): number => {
  switch (action.type) {
    case CREATE_TODO: {
      return state + 1;
    }
